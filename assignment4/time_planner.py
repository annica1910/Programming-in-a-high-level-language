...
def extract_events(table: bs4.element.Tag):
    ...
    for tr in rows:
        ...
        if len(cells) == 1:     #no events but a form of headline
            continue
        row = []
        for cell in cells:
            colspan = 1
...
def render_schedule(data: pd.DataFrame) -> str:
    ...
    for type_key in data.Type:
        if type_key:
            new_type_key = expand_event_type(type_key)
            data = data.replace(type_key, new_type_key)
...
def filter_data(keys: list, data: list, wanted: list):
    ...
    wanted_i = [keys.index(col) for col in wanted]

    for row_idx, row in enumerate(data):
        new_row = []
        for col_idx, cell in enumerate(row):
            if col_idx in wanted_i:
                new_row.append(strip_text(cell))     #remove unwanted text before adding cell
